...
overview: "Implement the core onboarding flow end-to-end: user type selection (Phase 0), calming welcome (Phase 1), static screener (Phase 2), account creation and clinician matching (Phase 3), with progress tracking and save/resume functionality. Development will proceed feature-by-feature with backend and frontend built in parallel."
todos:
  - id: models-migrations
    content: Create database models (User, Assessment, Clinician, Appointment) with migrations
    status: completed
  - id: auth-system
    content: Configure Devise with JWT authentication and create auth controllers
    status: completed
  - id: onboarding-layout
    content: Create onboarding layout, state store, and progress indicator
    status: completed
  - id: phase-0
    content: Build Phase 0 user type selection UI
    status: completed
  - id: phase-1
    content: Build Phase 1 welcome/regulate screen
    status: completed
  - id: phase-2
    content: Build Phase 2 static screener with PSC-17 form and API
    status: completed
  - id: phase-3a
    content: Build Phase 3A account creation with registration form
    status: completed
  - id: phase-3d
    content: Build Phase 3D clinician matching with seed data and display
    status: completed
  - id: save-resume
    content: Implement localStorage-based save and resume functionality
    status: completed
---
